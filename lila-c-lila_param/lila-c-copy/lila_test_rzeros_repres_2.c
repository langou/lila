...
int lila_test_rzeros_represe_2( int *lila_param, int m, int n, int i, int mt, double *A, int lda, double *T, int ldt, double *R, int ldr ){

	double norm_repres, *work, *RRi0, *Aii, norm_repres_2;
	int ml, lwork, ii, jj;
	...
	ml = m-i;
	info = LAPACKE_dlacpy_work( LAPACK_COL_MAJOR, 'A', ml, n, A, lda, RRi0, m );
	...
	work  = (double *) malloc(n * n * sizeof(double));
 	for(ii = 0; ii < n; ii++) for(jj = 0; jj < n; jj++) work[ ii+jj*n ] = Aii[ ii+jj*lda ] - RRi0[ ii+jj*m ];
