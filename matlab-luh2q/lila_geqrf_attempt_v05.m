...
%
   ...
   nl = nb(2);
%
%  [ A ] = lila_geqrf_v00( ml, nb(2), A, ilo(2), ilo(2), lda );
   [ A ] = lila_geqrf_v05( m, nb(2), ilo(2), A, 1, 1, lda, T, mt );
%
...
%
      ...
      jhit = jhit+vb;
%
      if ( jhit + mt > nb(2) ) vb = nb(2)-jhit+1; else vb = mt; end;
%
      ithi = itlo+vb-1;
      jthi = jtlo+vb-1;
%
   ...
   fprintf('ilot=%d, ihit=%d, jlot=%d, jhit=%d, vb=%d\n',itlo,ithi,jtlo,jthi,vb);
   TT(itlo:ithi,jtlo:jthi) = T(jtlo:jthi,jtlo:jthi);
%  if ( jthi + mt > nb(1) ) vt = nb(1) - jthi; else vt = mt; end;

%
% We have the previous T and the new T.
% Next step is to go and get the "connecting block".
%
      vb= mt - mod(ilo(2)-1,mt) ;
      if ( vb > nb(2) ), vb = nb(2); end
%
...
- ( TT(1:itlo-1,jtlo-(itlo)+1:jtlo-1) ...
     *A(jtlo:m,jtlo-(itlo)+1:jtlo-1)' )* ...
     ( ( tril(A(jtlo:m,jtlo:jthi),-1) + eye(size((A(jtlo:m,jtlo:jthi)))) ) ...
*TT(itlo:ithi,jtlo:jthi) );
