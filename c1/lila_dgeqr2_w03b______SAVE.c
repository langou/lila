...
//	}
	...
	for( i1 = 0; i1 < n ; i1++){
		if ( abs(1 - Asave[ i1 + i1*ml ]) < abs(1 + Asave[ i1 + i1*ml ]) ){
		...
		Asave[ i1 + i1*ml ] += 1.0e+00; 
		for( j1 = i1+1; j1 < ml; j1++ ) Asave[ j1 + i1*ml ] = Asave[ j1 + i1*ml ] / Asave[ i1 + i1*ml ];
 
//		for( j1 = i1+1; j1 < ml; j1++ ){
//			for( k = i1+1; k < n; k++){
//				Asave[ j1 + k*ml ] = Asave[ j1 + k*ml ] -  Asave[ j1 + i1*ml ] * Asave[ i1 + k*ml ]; 
//			}
//		}
		cblas_dgemm( CblasColMajor, CblasNoTrans, CblasNoTrans, ml-i1-1, n-i1-1, 1, (-1.0e+00), Asave + (i1+1) + i1*ml, ml, Asave + i1 + (i1+1)*ml, ml, (1.0e+00), Asave + (i1+1) + (i1+1)*ml, ml ); 
	...
	for( j1 = 0; j1 < n ; j1++){
		if( i1 <= j1 ){
			...
			printf("% 5.2e, ", Asave[ i1 + j1*ml ] - Aii[ i1 + j1*ml]);
		}
	}
	printf("\n");
	}
