...
//	printf("\n j = %d, n = %d, m = %d,\n",j,n,m);
	...
	for( k = 0; k < n; k++ ){
		...
		cblas_dgemm( CblasColMajor, CblasNoTrans, CblasNoTrans, n-k, n-k, 1, (-1.0e+00), Tkk+1, ldt, Tkk+ldt, ldt, (+1.0e+00), Tkk+1+ldt, ldt ); 
	...
	for( k = 0; k < n; k++ ){
		...
		if ( Sj[ k ] == -1 ){
			...
			//for( i1 = 0; i1 < m-j-n; i1++) Ajj[ i1 + n + k*lda ] = - Ajj[ i1 + n + k*lda ];
