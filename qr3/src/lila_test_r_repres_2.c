...
double lila_test_r_repres_2( int *lila_param, int m, int n, int i, int mt, double *A, int lda, double *T, int ldt, double *R, int ldr ){
	...
	double norm_repres_1, norm_repres_2, normA;
	double *work, *RR, *RRi0, *Rii, *Aii;
	...
	free( work );
	norm_repres_1 = LAPACKE_dlantr_work(LAPACK_COL_MAJOR, 'F', 'L', 'N', ml-1, n, RRi0+1, m, NULL );
	...
	return LAPACKE_dlapy2( norm_repres_1, norm_repres_2) / normA ;
}
