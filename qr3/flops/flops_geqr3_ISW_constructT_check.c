...
long int flops_geqr3_ISW_constructT_check( int int_m, int int_n ){
...
//		flops += flops_lapack_larfg( m );
		...
		flops += flops_geqr3_bef_constructT_check( m, n1 );
...
//		flops += n1 * n1 * n2;                             // 5: TRMM
		...
		flops += flops_geqr3_ISW_constructT_check( m-n1, n2 );
