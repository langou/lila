...
int main(int argc, char ** argv) {
	...
	qr2_dgeqr3R_ISW( m, n, Q, ldq, Q, ldq, R, ldr );
	for(i=0;i<n;i++) tau[i] = Q[i+i*ldq];
	qr2_dorgqr_VT2Q( m, n, Q, ldq, Q, ldq, tau );
	...
	// Need flop count for orgqr_VT2Q
//	perform_abs = ( ((double) flops_geqr3_ISW( m, n ) ) + ((double) flops_orgqr_VT2Q( m, n, n ) ) ) / elapsed / 1.0e+9 ;

	if ( verbose ){ 

		printf("QR3 ISW VT2Q ISW GEQRF ORGQR");
		printf("m = %4d, ", m);
		printf("n = %4d, ", n);
		printf(" \n");
