...
int main(int argc, char ** argv) {
	...
	// The below is missing the copy of the transpose and inverting tau for flops
	perform_abs = ( ((double) flops_syrk( m, n ) ) + ((double) flops_V2N( n ) ) ) / elapsed / 1.0e+9 ;

	if ( verbose ){ 

		printf("V2T V2N LARFT");
		printf("m = %4d, ", m);
		printf("n = %4d, ", n);
		printf(" \n");
