...
int dgeqr3R_ISW( int m, int n, double *A, int lda, double *T, int ldt, double *R, int ldr ){
	...
	if ( n == 1){
		...
		(*R) = (*A); // Need to copy R over first, at least when we point T and A at the same point ...
		(*T) = tau;
	...
	dgeqr3R( m, n1, A11, lda, T11, ldt, R11, ldr );
		...
		cblas_dtrmm ( CblasColMajor, CblasLeft, CblasUpper, CblasTrans, CblasNonUnit, n1, n2, 1.0e+00, T11, ldt, R12, ldr); 
	...
	dgeqr3R_ISW( m-n1, n2, A22, lda, T22, ldt, R22, ldr );
