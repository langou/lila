...
int main(int argc, char ** argv) {
	...
	qr2_aux_dV2tau( m, n, Q, ldq, tau );
	qr2_dorgqr3_VS2Q( m, n, Q, ldq, Q, ldq, tau );
...
//	perform_abs = ( ((double) flops_geqr3_UT_ISW( m, n ) ) + ((double) flops_orgqr_dVS2Q( m, n, n ) ) ) / elapsed / 1.0e+9 ;

	if ( verbose ){ 

		printf("QR3 UT VS2Q ISW GEQRF ORGQR");
		printf("m = %4d, ", m);
		printf("n = %4d, ", n);
		printf(" \n");
