...
// CHECK PERFORM_REF --- USING LAPACK TO COMPUTE FLOPS (NEED FLOPS FOR ISW && UT)
...
// 	*qr3_dorgqr* does not construct the nullspace of Q so the orthogonality check is wrong. Change from 'n' to 'k' ( in check ) and it should work
//	qr3_dorgqr( m, k, Q, ldq, T, ldt, tau );

//	A one-level blocking can be done here
