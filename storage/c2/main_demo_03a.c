...
int main(int argc, char ** argv){
	...
	int m, n, nb, vb, i, j, mt, lda, ldq, ldt, info, lwork;
	...
	info = lila_dgeqr2_w03a( m, vb, 0, mt, A, lda, T, ldt, Q, ldq, work, lwork );
	j += vb;
	if ( j+nb > n ) vb = n-j; else vb = nb;
	while( vb!=0 ){
	info = lila_dormqrf_w03( m, vb, j, 0, j, mt, A, lda, T, ldt, work, lwork );
	info = lila_dgeqr2_w03a( m, vb, j, mt, A, lda, T, ldt, Q, ldq, work, lwork );
	info = lila_dlarft_connect_w03(m, vb, j, 0, mt, A, lda, T, ldt );
	info = lila_dormqrbz_w03( m, vb, j, 0, j, mt, A, lda, Q, ldq, T, ldt, work, lwork );	
	j += vb;
	if ( j+nb > n ) vb = n-j; else vb = nb;
	}
...
////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	work = (double *) malloc(m * m * sizeof(double)); // need to creat z03 for this
	if( m > n )  lila_dormqrf_z03( m, n, n, 0, 0, mt, A, lda, RR, m, T, ldt, work, lwork );
	if( m == n )  lila_dormqrf_z03( m, n, n-1, 0, 0, mt, A, lda, RR, m, T, ldt, work, lwork );
